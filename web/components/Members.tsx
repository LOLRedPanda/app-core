...
function Members({teamData} : InferProps<typeof Members.propTypes>) {
  ...
  const memberItems = members.map((member: any) => <Member key={member.name} image={`/assets/RoleLogos/${member.position.toLowerCase()}.png`} name={member.name} position={member.position} />)
